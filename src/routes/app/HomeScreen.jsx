...
function HomeScreen() {
    ...
    return (
        <div className="flex flex-col items-center py-6 px-4">
            ...
            <SingleLineTextInput id="search" type="search" placeholder="search" onChange={filter}></SingleLineTextInput>
            <div className="justify-start">
                {foundDentists && foundDentists.length > 0 ? foundDentists.map(dentist => {
                    return(<DentistListItem firstName={dentist.firstName} lastName={dentist.lastName} number={dentist.number}/>)
                }): <div></div>}
            </div>
        </div>
    );
